<template>
  <div class="relative">
    ...
    <div class="overflow-hidden size-full" ref="carouselContainer">
      <div class="flex transition-transform duration-500 ease-in-out size-full" :style="slideStyle">
        <div v-for="(_, index) in slidesCount" :key="index" class="w-full flex-shrink-0">
    ...
    <template v-if="showArrows">
      ...
      <button @click="nextSlide"
        ...
        :class="{ 'opacity-50 cursor-not-allowed': !loop && currentIndex === slidesCount - 1 }">
    ...
    <div v-if="showDots" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
      <button v-for="index in slidesCount" :key="index - 1" @click="goToSlide(index - 1)"
        class="w-2 h-2 rounded-full transition-all"
        :class="(index - 1) === currentIndex ? 'bg-primary w-4' : 'bg-white/60 hover:bg-white/80'">
      </button>
    </div>
  </div>
</template>
...
// Calcul du nombre de slides en fonction des slots nommés
const slidesCount = computed(() => {
  return Object.keys(slots).filter(name => name.startsWith('slide-')).length;
});
