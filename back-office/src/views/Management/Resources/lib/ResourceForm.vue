<template>
    <div v-if="hasAtLeastOneSalon && isReady" class="grid grid-cols-1 gap-8 m-6 sm:grid-cols-2">
        <CoelhoInputGroup v-model="localResource.type" label="Tipo de recurso" component="select"
            :error="validationErrors.type" :options="resourceTypesOptions" :required="true" />
        <CoelhoInputGroup v-model="localResource.salon" label="Salão" component="select" :searchable="true"
...
import { CoelhoButton, CoelhoInputGroup, CoelhoInput } from '@/components';
import { ChatBubbleLeftEllipsisIcon, PlusCircleIcon } from '@heroicons/vue/24/solid';
